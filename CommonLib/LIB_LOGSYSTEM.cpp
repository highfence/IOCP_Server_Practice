...
const BOOL LIB_LOGSYSTEM::InitialLogWrite(const BYTE bLogSystem, const LPCTSTR szFileName, const LPCTSTR szEventLogName)
{
	...
	m_Lock.Lock();
	{
		if ( wcslen(szEventLogName) <= 1 )
			...
			wcscpy(m_szEventLogName, szEventLogName);
...
const BOOL LIB_LOGSYSTEM::InitialLogFile(const LPCWSTR szFileName)
{
	m_Lock.Lock();
	{
		// WARNING :: char*로 마음대로 캐스팅함.
		m_pFile = fopen((char*)szFileName, "a");
...
const BOOL LIB_LOGSYSTEM::WriteLogFile(const LPCWSTR szWrite)
{
	m_Lock.Lock();
	{
		fwrite(szWrite, sizeof(TCHAR), wcslen(szWrite), m_pFile);
...
const BOOL LIB_LOGSYSTEM::WriteLogHandle(const LPCWSTR szWrite)
{
	DWORD dwWritten = 0;

	m_Lock.Lock();
	{
		if ( WriteFile(m_hFile, szWrite, (DWORD)wcslen(szWrite), &dwWritten, NULL) != TRUE )
			return FALSE;
	}
	m_Lock.UnLock();
...
const VOID LIB_LOGSYSTEM::EventLog(int nLoop, LPCWSTR pFormat, ...)
{
